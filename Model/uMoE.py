...
class MoE(): 
    ...
    def fit(self, train_data, train_target, valid_data = None, 
        ...
        if valid_data is not None:
            labels_valid = self.pred_clusters(valid_data.sample(n_samples,seed = seed, threshold = threshold_samples))
            prob_dist_valid = self.__prob_mass_cluster(self.n_experts, labels_valid, int(n_samples * threshold_samples))
            if local_mode == True:
                valid_data_local =  self.__local_cluster_mode(valid_data, prob_dist_valid)
                valid_loader_list_experts = self.__divide_dataset_for_experts(valid_data_local, valid_target, prob_dist_valid, self.n_experts, batch_size = batch_size_experts, weighted_experts = weighted_experts)
            else:
                valid_loader_list_experts = self.__divide_dataset_for_experts(valid_data.mode(), valid_target, prob_dist_valid, self.n_experts, batch_size = batch_size_experts, weighted_experts = weighted_experts)
        else:
        ...
        for i in range(self.n_experts):
            ...
            self.experts[i].train_model(train_loader = train_loader_list_experts[i], valid_loader = valid_loader_list_experts[i],
    ...
    def __local_cluster_mode(self, data, prob_dist):
        ...
        for i, instance in tqdm(enumerate(data_local_mode), total=len(data_local_mode), desc="Search Local Mode of Subspace"):
            ...
            if len(missing_dims) > 0:
                ...
                with warnings.catch_warnings():
                    ...
                    optimize_modal = basinhopping(lambda x: objective_function(x = x, cluster = cluster, kde = data.data[i], instance = instance, missing_dims = missing_dims, centroids = self.kmeans.cluster_centers_), x0=cluster_centers, minimizer_kwargs=minimizer_kwargs, niter=25, stepsize=0.2)                                    
            ...
            if valid_loader is not None:
                ...
                with torch.no_grad():
                    ...
                    if self.task in (1,3):
                        ...
                        loss_val = torch.mean(loss_val * weights_val)
                    else:
                        loss_val = loss_fn(y_preds, y_targets)
                        loss_val = torch.mean(loss_val * weights_val.unsqueeze(1))
...
class Gate_nn(nn.Module):
    ...
    def train_model(self, train_loader_expert, train_loader_gate, valid_loader_expert, valid_loader_gate, n_epochs, loss_fn, weighted_loss, lr, reg_alpha, reg_lambda, verbose, task):
        ...
        for epoch in range(n_epochs):
            ...
            if valid_loader_gate is not None:
                ...
                with torch.no_grad():
                    ...
                    loss_val = loss_fn(y_preds, y_targets)
                    loss_val = float(loss_val.mean())
                    history_val_weighted.append(loss_val)
                    if loss_val < best_score:
                        best_score = loss_val
                        best_weights = copy.deepcopy(self.state_dict())
            if (verbose == True) and (epoch % 5 == 0):
        ...
        if valid_loader_gate is not None:
            self.load_state_dict(best_weights)
        return history_val_weighted, best_score
